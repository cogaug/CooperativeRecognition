...
class Expansion():
    def __init__(
        ...
        self.gc1 = GenerateCar(
            ...
            init_local = [-61.5, 254.5, -0.04660,90],
        ...
        self.gc2 = GenerateCar(
            ...
            init_local = [-66.1,  251., -0.04660,90],
        ...
        self.gc3 = GenerateCar(
            ...
            init_local = [-70.6, 247.5, -0.04660,90],
        ...
        self.num_walker_pose = [[523.,238,90], [526.8,242.0,90], [529.0,244.4,90]]
    ...
    def run_loop(self):
        try:
            ...
            gc1_vehicle, gc1_camera, gc1_depth_camera, gc1_bbox = self.gc1.setting(self.gc1.model,self.gc1.init_local,world)
            gc2_vehicle, gc2_camera, gc2_depth_camera, gc2_bbox = self.gc2.setting(self.gc2.model,self.gc2.init_local,world)
            gc3_vehicle, gc3_camera, gc3_depth_camera, gc3_bbox = self.gc3.setting(self.gc3.model,self.gc3.init_local,world)
            ...
            while True:
                ...
                if not self.multi_streaming:
                    if self.streaming_id<4:
                        frame = gc_queue_id[self.streaming_id-1].get()
                        frame = np.reshape(np.copy(frame.raw_data), (frame.height, frame.width, 4))

                        depth_frame = gc_depth_queue_id[self.streaming_id-1].get()
                    ...
                    frame,detect_info=self.detection.make_box(frame, depth_frame,gc_camera[self.streaming_id-1].get_transform())
                    if self.streaming_id==2:
                        match_results = self.cs_car2.matching(detected_info=detect_info,other_car = [location_car1,location_car3])
                    if self.streaming_id==3:
                        match_results = self.cs_car3.matching(detected_info=detect_info,other_car = [location_car1,location_car2])
                ...
                if self.show_graph:
                    ...
                    self.sg.generate_graph([self.cs_car3.own_info+[[location_car3.x,location_car3.y]]]+match_results)
